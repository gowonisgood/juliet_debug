...
static int iwasm_instance_exec_main(wasm_module_inst_t module_inst, int argc, char **argv)
{
    ...
    if ((exception = wasm_runtime_get_exception(module_inst))) {
        puts(exception);
	/*GO : example code */
	/*if (strstr(exception, "import function") != NULL) {
         //not_import++;
	 //except_case++;
         //printf("import_count:%d except_case: %d\n", not_import, except_case);
	 return -2; //if exception case : return -2
       }*/
	if(strstr(exception, "hashpt") !=NULL){
		bad_catch++;
		printf("bad_catch:%d\n", bad_catch);
		return -1;
	}
        //return -1; //GO: original code
	return -2;
    }
    if (argc > 0) {
        ret = *((int*)argv);
...
int iwasm_bytecode_exec_main(uint8_t *bytecode, size_t bytecode_len, int argc, char **argv)
{
    ...
    {   /* load WASM module */
	    ...
	    /* GO */
	   /*if (strstr(error_buf, "zero byte") != NULL) {
         	zero_byte++;
         	except_case++;
        	printf("zero_byte:%d except_case: %d\n", zero_byte, except_case);
       	    }*/
...
int wamr_run_cp(const void *bytecode, size_t bytecode_len, int argc, char *argv[])
{
    ...
    free(wasm_buf);
    /* GO */
    //printf("total cases(total cases-except cases): %d , bad cases: %d , catched badcases: %d  \n", (cases-except_case), (cases-except_case)/2 ,bad_catch);
    return ret;
}
